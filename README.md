...
1. Create a Vivado project and add the sources in `rtl/` and the testbench you want to run from `tb/`.
2. Copy the contents of the corresponding file in `programs/` into the file that `instr_mem.v` loads via `$readmemh`, keeping the filename unchanged.
3. Set the testbench as simulation top and run behavioural simulation.
4. Check the console output for the pass/fail summary.

> **Note on `$readmemh` filenames.** On Windows, Notepad appends `.txt` silently, so `program.hex` may actually be `program.hex.txt`. The filename in `$readmemh` must match exactly, or use an absolute path.
...
- **No `SLT`.** Magnitude comparison is not directly available; loops must test equality with `BEQ` or count down to zero. As a consequence `ALUControl` needs only 2 bits rather than the usual 3. Algorithms like finding a maximum or sorting are awkward to express.
- **No `JALR`.** `JAL` correctly saves the return address (verified: `x1 = PC + 4`), but without `JALR` that address cannot be used to return, so `JAL` currently serves only as an unconditional jump.
- **Data memory is not preloaded.** `data_mem.v` has no `$readmemh`, so array-based programs must store their own input data with `SW` before reading it back with `LW`.
- **No alignment checking.** A misaligned access (address not a multiple of 4) is silently truncated by `A[7:2]` rather than raising an exception.
